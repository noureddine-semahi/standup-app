...
export default function AssistantPanel({ onClose, onActionTaken }: AssistantPanelProps) {
  ...
  async function postToAssistant(payload: Record<string, any>) {
    ...
    if (!accessToken) throw new Error(t("assistant.notSignedIn"));
    ...
    if (!res.ok) throw new Error(data.message ?? t("assistant.somethingWentWrong"));
    return data;
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      setError(e?.message ?? t("assistant.somethingWentWrong"));
    } finally {
      setSending(false);
    }
  }
      ...
      >
        ...
        <div className="relative">
          <textarea
            ...
            disabled={sending}
            placeholder={listening ? t("assistant.listeningPlaceholder") : t("assistant.inputPlaceholder")}
            autoFocus
            rows={2}
          ...
          {voiceSupported && (
            <button
              ...
              disabled={sending}
              title={listening ? t("assistant.stopListening") : t("assistant.speakInstead")}
        ...
        {typeof lastResponse?.remaining === "number" && (
          <div className="mt-3 text-xs text-white/40">{t("assistant.remainingActions", { count: lastResponse.remaining })}</div>
        )}
      </div>
    </div>
  );
